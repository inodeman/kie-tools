...
import { SWEditor, SWEditorImpl } from "./SWEditor";
import { SWEditorChannelApi, getSWLanguageData } from "../api";
...
export class SWEditorFactory implements EditorFactory<SWEditor, SWEditorChannelApi> {
  constructor(private readonly gwtEditorEnvelopeConfig: { shouldLoadResourcesDynamically: boolean }) {}

  public createEditor(
    ctx: KogitoEditorEnvelopeContextType<SWEditorChannelApi>,
    initArgs: EditorInitArgs
  ): Promise<SWEditor> {
    const languageData = getSWLanguageData(initArgs.resourcesPathPrefix);
    const factory = new GwtEditorWrapperFactory<SWEditor>(
      languageData,
      (self) =>
        new SWEditorImpl(
